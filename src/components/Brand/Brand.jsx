          ...
          {brands.map((brand) => (
            <div key={brand._id} className="w-full lg:w-1/4 md:w-1/3 sm:w-1/2 p-3">
              <div className="brand rounded-lg shadow-2xl overflow-hidden cursor-pointer">
                <img
  className="h-[200px] object-cover transform transition-transform duration-300 hover:scale-110 hover:shadow-xl hover:shadow-emerald-500"
  src={brand.image}
                  alt={brand.name}
                />
                <h2 className="m-3 text-emerald-500 text-2xl font-bold">
                  {brand.name}
                </h2>
              </div>
            </div>
